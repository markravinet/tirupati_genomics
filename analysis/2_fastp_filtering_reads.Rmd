...
First we check fastp works and set up our data for analysis.
...
cp /home/data/reads/*.fastq.gz ./
```
Next we set up some variables:

```shell
FORWARD=PDOM2023IND0001J_30707403185-HS1_S178_L002_R1_001.fastq.gz
REVERSE=PDOM2023IND0001J_30707403185-HS1_S178_L002_R2_001.fastq.gz
```
...
```shell
# Run fastp
fastp --in1 $FORWARD --in2 $REVERSE --out1 ${FORWARD%.*.*}.trimmed.fastq.gz --out2 ${REVERSE%.*.*}.trimmed.fastq.gz -l 50 -h ${FORWARD%%_*}.html &> ${FORWARD%%_*}.log
